...
Settings live in `%APPDATA%\Shout\config.json`. The cue lab creates the file the first time you save a sound; otherwise, create it yourself. Any key you leave out keeps its default. Quit and restart Shout after editing. Changes made in the cue lab are the exception: they apply from the next dictation.
...
| **Clipboard** | Text is pasted via the clipboard and marked private, so it stays out of Windows Clipboard History and cloud clipboard sync. Your previous clipboard is put back, with its own privacy intact. |
